...
def entrypoint(debug: str = "") -> None:
    ...
    """
    # MODIFIED: Import YOLO11CBAM for custom model support
    from ultralytics.nn.yolo11_cbam import YOLO11CBAM  # Added import for custom CBAM model
    # END MODIFIED
    ...
    model_type = overrides.pop("model_type", None)  # Added to handle custom model type
    if model is None:
        model = "yolo11n.pt"
        LOGGER.warning(f"'model' argument is missing. Using default 'model={model}'.")
    overrides["model"] = model
    stem = Path(model).stem.lower()
    # MODIFIED: Add support for YOLO11CBAM
    if model_type == "yolo11cbam":  # Check for custom YOLO11CBAM model
        model = YOLO11CBAM(model, task=task)
    # END MODIFIED
